...
ggplot(total, aes(x=energy, y=valence, col=playlist_name, size=(track.duration_ms/1000)/60)) + geom_point(alpha=0.4) + facet_wrap(facet="name", nrow=1, ncol=13, shrink = TRUE) + scale_color_manual(values= c("#03b5fc", "#bf0d00")) + scale_x_continuous(breaks=c(0, 0.5, 1)) + theme(legend.position="bottom", strip.text.x = element_text(angle = 90)) + labs(x="Energy", y="Valence", size="Duration in min", col="")
